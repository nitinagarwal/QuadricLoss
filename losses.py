...
def NN_loss(V1, V2, dim=0):
    dist = pairwise_dist(V1, V2)
...
if __name__ == "__main__":
    ...
    """testing chamferLoss"""
    # meshA = '/home/minions/Dropbox/GraphicsLab/Projects/3D_Content_Creation/data/all_data/cubes/harrington_0_13_17.obj'
    # V, F = load_obj_data(meshA)
    # # adj = get_adjacency_matrix(V, F, 20)
    ...
    # meshB = '/home/minions/Dropbox/GraphicsLab/Projects/3D_Content_Creation/data/all_data/cubes/harrington_0_6_20.obj'
    # Vb, Fb = load_obj_data(meshB)
    # # adjb = get_adjacency_matrix(V, F, 20)
    # Vb = torch.from_numpy(Vb.astype(np.float32))
    # # adjb = torch.from_numpy(adjb.astype(np.float32))
    # Vb.requires_grad_()
    # # adjb.requires_grad_()
    # Vb = Vb.to('cuda')
    # # adjb = adjb.to('cuda')
    # Vb = torch.unsqueeze(Vb, 0)

    # loss, _, _ = chamferLoss(V, Vb)
    # print(loss)
    # loss.backward()
    # print(loss.item())
    ...
    adj = adj.unsqueeze(0)
    Nin = Nin.unsqueeze(0)
    corres = torch.rand(1,2500,3)
    
    V.requires_grad_()
    adj.requires_grad_()
    Nin.requires_grad_()
    corres.requires_grad_()

    V = V.to('cuda')
    adj = adj.to('cuda')
    Nin = Nin.to('cuda')
    corres = corres.to('cuda')
